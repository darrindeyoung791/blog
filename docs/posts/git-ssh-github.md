...
部分情况下，我们能直接访问 [GitHub](https://github.com)，但有时即便浏览器能够正确访问 GitHub，git 却没办法连接 GitHub。使用 SSH 连接 GitHub 可以解决此类问题。

::: tip
本文参考了 https://zhuanlan.zhihu.com/p/688103044 以及 https://blog.csdn.net/problc/article/tip/144365441 的内容。
:::

## 配置 SSH key

> 需要提前配置好 GitHub 账号、git 用户名、邮箱等
...
::: tip 如果报错 `no such file or directory`
...
::: tip Windows
在 Windows 下，SSH 配置文件（config）通常位于用户目录的 `.ssh` 文件夹中（例如：`C:\Users\darri\.ssh\config`）。如果没有就创建一个。
